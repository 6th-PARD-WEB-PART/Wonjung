    ...
    <header className="sticky top-0 z-10 w-full py-2.5 bg-white border-b border-gray-200">
      <div className="max-w-6xl mx-auto flex justify-between items-center h-14 px-4 sm:px-6">
        <div className="flex justify-start items-center h-10">
          <Link href="/">
            <Image src="/carrot.png" alt="당근마켓로고" className="max-w-16 cursor-pointer" width={64} height={40}/>
          </Link>

          {/* 로고 옆 글자 */}
          <nav className="hidden sm:flex ml-8 space-x-6 text-gray-700 text-base font-medium">
            {['중고거래', '부동산', '중고차', '알바', '동네업체', '동네생활', '모임'].map((item) =>(
              <Link href={`/${item}`} key={item} className="hover:text-orange-500 transition">
                {item}
              </Link>
            ))}
          </nav>
        ...
        <div className="flex justify-start items-center space-x-4">
            <div className="flex items-center cursor-pointer hover:bg-gray-100 p-2 rounded transition">
              <Image src="/point.png" alt="위치아이콘" width={16} height={16} className="w-4 h-4"/>
              <span className="ml-2 text-gray-700 text-sm font-normal">송도동</span>
              <Image src="/down.png" alt="아래표시 아이콘" width={16} height={16} className="w-4 h-4"/>
